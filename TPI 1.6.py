...
# --- Helper Functions (These would normally be in a separate module) ---
...
def create_dag(dag_id, config, data_url_config):
    ...
    dag = DAG(
        ...
        schedule_interval=config.get('schedule_interval', '@hourly'),  # Use get for safety
...
# --- DAG Creation Logic (within the same file) ---

@provide_session
def create_dags_from_config(config_file="config.json", session=None):
    """
    Loads config and creates DAGs, run as an Airflow task.
    """
    try:
    ...
    for data_url_config in config['data_urls']:
        ...
        if not DagRun.query.filter(DagRun.dag_id == dag_id).first():
            dag = create_dag(dag_id, config, data_url_config)
            globals()[dag_id] = dag  # Make DAG available to Airflow
